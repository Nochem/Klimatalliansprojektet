...
<div id="wrapper">
    ...
    <div id="content">
        <form method="get" name="form" id="form">
            ...
            $editID = $_SESSION['Id'];
            // -------------- Transport ------------
            ////Hämtar alla utsläppskällor för lokaler som inte finns i transport
            if ($noReportTransSql = mysqli_prepare($dbc, "SELECT EmissionSource,Unit,ConvFactor,EmissionCO2perMWh FROM ConversionFactors WHERE EmissionSource NOT IN (SELECT EmissionSource FROM Transport WHERE ID = ?) AND Category = ?")) {
                $noReportTransSql->bind_param("is", $editID,$categoryTransport);
                $noReportTransSql->execute();
                $noReportTransSqlresult = $noReportTransSql->get_result();
            }
            //Hämtar alla utsläppskällor för lokaler som inte finns i Lokaler
            if ($noReportPlaceSql = mysqli_prepare($dbc, "SELECT EmissionSource,Unit,ConvFactor,EmissionCO2perMWh FROM ConversionFactors WHERE EmissionSource NOT IN (SELECT EmissionSource FROM Transport WHERE ID = ?) AND Category = ?")) {
                $noReportPlaceSql->bind_param("is", $editID ,$categoryLokalerProcesser);
                $noReportPlaceSql->execute();
                $noReportPlaceSqlresult = $noReportPlaceSql->get_result();
            }
            //Hämtar rapporterad data från transport
            if ($editTransportSQL = mysqli_prepare($dbc, "SELECT EmissionSource, Amount,Unit, ConvFactor,EmissionMwh, TonCO2 from Transport,Report Where Transport.Id = ? and Transport.id = Report.id and Report.User = ?")) {
                $editTransportSQL->bind_param("is",$editID , $login_session);
                $editTransportSQL->execute();
                $editTransportSQLresult = $editTransportSQL->get_result();
            }
            //Hämtar rapporterad data från lokaler
            if ($editPlacesSQL = mysqli_prepare($dbc, "SELECT EmissionSource, Amount,Unit, ConvFactor,EmissionMwh, TonCO2 from PlacesAndProcesses,Report Where PlacesAndProcesses.Id = ? and PlacesAndProcesses.id = Report.id and Report.User = ?")) {
                $editPlacesSQL->bind_param("is",$editID , $login_session);
                $editPlacesSQL->execute();
                $editPlacesSQLresult = $editPlacesSQL->get_result();
            }
            if ($unitCheckTrans = mysqli_prepare($dbc, "SELECT EmissionSource,Unit FROM ConversionFactors where Category = ?")) {
                $unitCheckTrans->bind_param("s",$categoryTransport);
                $unitCheckTrans->execute();
                $unitCheckTransRes = $unitCheckTrans->get_result();
            }
            if ($unitCheckPlaces = mysqli_prepare($dbc, "SELECT EmissionSource,Unit FROM ConversionFactors where Category = ?")) {
                $unitCheckPlaces->bind_param("s",$categoryLokalerProcesser);
                $unitCheckPlaces->execute();
                $unitCheckPlacesRes = $unitCheckPlaces->get_result();
            }
            ...
            echo '</tr>';
            while ($reportedtransportRow = $editTransportSQLresult->fetch_assoc()) {
                if(!empty($reportedtransportRow)){
                    $transportcount++;
                    echo '<tr>';
                    echo '<td>' .$reportedtransportRow['EmissionSource']. '</td>';
                    $str = htmlspecialchars($reportedtransportRow['EmissionSource']);
                    echo "<input type=\"hidden\" name=\"emissionSource[]\" value=\"$str\">";
                    echo '<td> <input type="text" value = '.$reportedtransportRow['Amount'].' name="amount[]" oninput="tonCO2(' . $arrayindex . ')"onchange ="tonCO2(' . $arrayindex . ')"class="inputbox"/></td>';
                    // Kör TonCO2 funktionen för att den ska funka från början
                    echo '<script type="text/javascript">', 'tonCO2(' . $arrayindex . ');', '</script>';

                    //Skapar Select boxen för unit
                    echo '<td>';
                    echo '<select name="unit[]" onchange="selectedUnit(' . $arrayindex . ')">';
                    // SELECT BOXENS INNEHÅLL GENERERAS HÄR
                    if($unitCheckTransRes){
                        $filled  = false;
                        //För varje unit i conversionfactors som tillhör transport
                        $cmp = htmlspecialchars($reportedtransportRow['EmissionSource']); // CMP är stringar man jämför med
                    while($unitCheck = $unitCheckTransRes->fetch_assoc()){
                       $cmp2 =  htmlspecialchars($unitCheck['EmissionSource']);
                        if( strcmp ($cmp ,$cmp2 ) == 0){
                            $cmp3 = htmlspecialchars($reportedtransportRow['Unit']);
                            $cmp4 =  htmlspecialchars($unitCheck['Unit']);
                            //Om valda unit är samma som standard i conversion factors.
                    if(strcmp($cmp3,$cmp4)==0){
                    echo '<option value =' . $reportedtransportRow['Unit'] . '>' . $reportedtransportRow['Unit'] .  '</option>';
                    echo '<option value ="Ton"> Ton </option>';
                    $filled = true;
                        }else{
                        echo '<option value ="Ton"> Ton </option>';
                        echo '<option value =' . $unitCheck['Unit'] . '>' . $unitCheck['Unit'] .  '</option>';
                        $filled = true;
                    }
                        }
                    }
                    ...
                    echo '</select>';
                    //resetar resultsetet så att den börjar på rad 0;
                    mysqli_data_seek($unitCheckTransRes, 0);
                    echo '</td>';
                    // HÄR SLUTAR SELECTBOXEN
                    echo '<td ><p name = "coFactor[]"> ' . $reportedtransportRow['ConvFactor'] . '</p></td>';
                    echo '<input type="hidden" name="convFactor[]" value=' . $reportedtransportRow['convFactor'] . '>';
                    echo '<td style="display:none;" id= >' . $reportedtransportRow['EmissionMWh'] . '</td>';
                    echo '<input type="hidden" name="emissionCO2[]" value=' . $reportedtransportRow['EmissionMWh'] . '>';
                    echo '<td name="tonCO[]">';
                    echo $reportedtransportRow['TonCO2'];
                    echo '</td>';
            ...
            while ($notReportedRow = $noReportTransSqlresult->fetch_assoc()) {
                if (!empty($notReportedRow)) {
                    ...
                    echo '<td >';
                    echo $notReportedRow['EmissionSource'];
                    echo '</td>';
                    $str = htmlspecialchars($notReportedRow['EmissionSource']);
				...
				class="inputbox"/>';
                    ...
                    echo '<select name="unit[]" onchange="selectedUnit(' . $arrayindex . ')">';
                    echo '<option value =' . $notReportedRow['Unit'] . '>' . $notReportedRow['Unit'] .  '</option>';
                    echo '<option value ="Ton"> Ton </option>';
                    echo '</select>';
                    echo '</td>';
                    //skapar omräkningsfaktor
                    echo '<td class="colCenter">';
                    echo '<p name = "coFactor[]"> ' . $notReportedRow['ConvFactor'] . '</p>';
                    echo '</td>';
                    echo '<input type="hidden" name="convFactor[]" value=' . $notReportedRow['ConvFactor'] . '>';
                    //skapar utsläpp i mwh
                    echo '<td style="display:none;" id= >' . $notReportedRow['EmissionCO2perMWh'] . '</td>';
                    echo '<input type="hidden" name="emissionCO2[]" value=' . $notReportedRow['EmissionCO2perMWh'] . '>';
