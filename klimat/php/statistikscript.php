...
$sql2 = "select a.Year, a.User, ( ifnull(sum(c.TotalAmount)/1000,0) + ifnull(sum(b.KgCO2)/1000,0) ) as TonCO2 from Report as a left join Flights as b on a.id = b.id left join OtherFlight as c on a.id = c.id where a.user = '{$row['Name']}' group by a.user, a.year";
